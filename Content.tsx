...
// FIX: Import PointerEvent from react to fix missing namespace error.
import {PointerEvent, useCallback, useEffect, useMemo, useRef, useState} from 'react';
import {ResizePayload, useResizeDetector} from 'react-resize-detector';
import {
  ActiveColorAtom,
  FilteredBoundingBoxes2DAtom,
  FilteredBoundingBoxes3DAtom,
  FilteredBoundingBoxMasksAtom,
  FilteredPointsAtom,
  DetectTypeAtom,
  DrawModeAtom,
  FOVAtom,
...
export function Content() {
  ...
  const [boundingBoxes2D] = useAtom(FilteredBoundingBoxes2DAtom);
  const [boundingBoxes3D] = useAtom(FilteredBoundingBoxes3DAtom);
  const [boundingBoxMasks] = useAtom(FilteredBoundingBoxMasksAtom);
  const [stream] = useAtom(ShareStream);
  const [detectType] = useAtom(DetectTypeAtom);
  const [videoRef] = useAtom(VideoRefAtom);
  const [fov] = useAtom(FOVAtom);
  const [, setImageSent] = useAtom(ImageSentAtom);
  const [points] = useAtom(FilteredPointsAtom);
  ...
  const [isScreenshare] = useAtom(IsScreenshareAtom);
