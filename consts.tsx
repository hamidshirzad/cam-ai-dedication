...
export const defaultPromptParts = {
  ...
  'Segmentation masks': [
    ...
    'all objects',
    `. Output a JSON list of segmentation masks where each entry contains the 2D bounding box in the key "box_2d", the segmentation mask in key "mask", a confidence score between 0.0 and 1.0 in "confidence", and the text label in the key "label". Use descriptive labels.`,
  ],
  '3D bounding boxes': [
    'Output in json. Detect the 3D bounding boxes of ',
    'items',
    ', output no more than 10 items. Return a list where each entry contains the object name in "label", its 3D bounding box in "box_3d", and a confidence score between 0.0 and 1.0 in "confidence".',
  ],
  Points: [
    'Point to the',
    'items',
    ' with no more than 10 items. The answer should follow the json format: [{"point": <point>, "label": <label1>, "confidence": <score>}, ...]. The points are in [y, x] format normalized to 0-1000.',
  ],
};
